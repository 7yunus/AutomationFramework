...
public class ProductListPage {
  ...
  public boolean getItemPrices() {
    List<WebElement> productPrices =
        getDriver().findElements(By.xpath("//div[@class='pricebar']/div"));
    int size = productPrices.size();
    for (int i = 1; i < size; i++) {
      WebElement productPrice =
          getDriver().findElement(By.xpath("(//div[@class='pricebar']/div)[" + i + "]"));
      Double listingPrice = Double.parseDouble(productPrice.getText().replace("$", "")); //$29.99
      getDriver().findElement(By.xpath(
          "(//div[contains(@class,'inventory_item_price') and" + " text()='" + listingPrice
              + "']//preceding::a/img)[" + i + "]")).click();
